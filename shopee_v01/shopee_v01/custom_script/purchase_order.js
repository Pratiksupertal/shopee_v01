...
before_load:function(frm) {
  ...
  var df2=frappe.meta.get_docfield("Purchase Order Item", "amount",frm.doc.name);
  var df3=frappe.meta.get_docfield("Purchase Order Item", "last_purchase_rate",frm.doc.name);
  var df4=frappe.meta.get_docfield("Payment Schedule", "payment_amount",frm.doc.name);
  var df5=frappe.meta.get_docfield("Payment Schedule", "paid_amount",frm.doc.name);
  var df6=frappe.meta.get_docfield("Purchase Taxes and Charges", "tax_amount",frm.doc.name);
  var df7=frappe.meta.get_docfield("Purchase Taxes and Charges", "base_tax_amount",frm.doc.name);
  var df8=frappe.meta.get_docfield("Purchase Taxes and Charges", "total",frm.doc.name);
  var df9=frappe.meta.get_docfield("Purchase Taxes and Charges", "tax_amount_after_discount_amount",frm.doc.name);
  var df10=frappe.meta.get_docfield("Purchase Taxes and Charges", "base_tax_amount_after_discount_amount",frm.doc.name);
  var df11=frappe.meta.get_docfield("Purchase Order Item", "price_list_rate",frm.doc.name);
  var df12=frappe.meta.get_docfield("Purchase Order Item", "discount_amount",frm.doc.name);
  var df13=frappe.meta.get_docfield("Purchase Order Item", "net_rate",frm.doc.name);
  var df14=frappe.meta.get_docfield("Purchase Order Item", "net_amount",frm.doc.name);
  var df15=frappe.meta.get_docfield("Purchase Order Item", "billed_amt",frm.doc.name);
  var df16=frappe.meta.get_docfield("Purchase Order Item", "blanket_order_rate",frm.doc.name);

  if (frappe.user_roles.includes("ISS Purchase User") || frappe.user_roles.includes("Accounting Supervisor")
  || frappe.user_roles.includes("CEO") || frappe.user_roles.includes("Purchase AP")
