...
'''Creating Pick List for Purchase Order.'''
@frappe.whitelist()
def purchase_order_data(main_work_order):
	...
	number_of_items = len(purchase_orders)
	for purchase_order in purchase_orders:
		...
		for row in rows:
			all_items.append(row.item_name)
		# purchase_order["number"] = len(all_items)
		purchase_order["total_qty"] = purchase_order["total_qty"]/len(all_items)
	return purchase_orders


def make_pick_list_purchase_order(purchase_order_id, qty):
	try:
		data_validation_for_creating_pick_list_purchase_order(purchase_order_id, qty)
		all_items = []
		raw_materials_items = []
		not_raw_materials_items = []
		...
		max_qty = int(purchase_order_doc.total_qty)/len(all_items)
		picklist1 = generate_new_pick_list_purchase_order(raw_materials_items, purchase_order_doc, int(max_qty))
		picklist2 = generate_new_pick_list_purchase_order(not_raw_materials_items, purchase_order_doc, int(max_qty))
		response = f"For Purchase Order <strong><a href='{purchase_order_doc.get_url()}'>{purchase_order_doc.name}</a></strong>"
		if picklist1:
			response += f"<br>Pick List <strong><a href='{picklist1.get_url()}'>{picklist1.get('name')}</a></strong> is created for Raw Materials Items"
		if picklist2:
			response += f"<br>Pick List <strong><a href='{picklist2.get_url()}'>{picklist2.get('name')}</a></strong> is created for Accessories Items"
		...
		purchase_order_doc = frappe.get_doc('Purchase Order', purchase_order_id)
		# return _("Pick List not created for Work Order - <strong>{0}</strong>. Reason - {1}").format(get_link_to_form("Pick List", picklist1), str(e))
		return f"Pick List not created for Purchase Order - <strong><a href='{purchase_order_doc.get_url()}'>{purchase_order_doc.name}</a></strong>. Reason - {str(e)}"


def generate_new_pick_list_purchase_order(item_list, purchase_order_doc, max_qty):
	if not item_list: return
	pick_list = frappe.new_doc('Pick List')
	pick_list.company = purchase_order_doc.company
	pick_list.purpose = "Material Transfer for Manufacture"
	pick_list.purchase_order = purchase_order_doc.name
	pick_list.for_qty = max_qty
	for item in item_list:
		row = pick_list.append('locations', {})
		row.item_code = item
		row.warehouse = "Stores - ISS"
		row.qty = max_qty
		row.stock_qty = max_qty
		row.picked_qty = max_qty
	pick_list.save()
	return pick_list


@frappe.whitelist()
def purchase_order_pick_lists(purchase_order_list):
	purchase_order_list = json.loads(purchase_order_list)
	result = []
	for purchase_order in purchase_order_list:
		if "__checked" in purchase_order.keys():
			if purchase_order["__checked"]:
				res = make_pick_list_purchase_order(purchase_order_id=purchase_order["name"],qty=purchase_order["total_qty"])
				result.append(res)
	response_msg = "<br>".join(result)
	if len(response_msg) == 0:
		frappe.msgprint("Operation failed. No Purchase Order is selected.")
	else:
		frappe.msgprint(response_msg)


def data_validation_for_creating_pick_list_purchase_order(purchase_order, qty):
	max_finished_goods_qty = frappe.db.get_value('Purchase Order', purchase_order, 'total_qty')
	if qty != max_finished_goods_qty:
		raise Exception("Input quantity is not equal to the total quantity")
