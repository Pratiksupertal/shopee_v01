...
def get_entries(filters):
    ...
    if filters.get("warehouse_name") == "[901] - Finished - ISS":
        ...
        inner join `tabItem Variant Attribute` iva on b.item_code = iva.parent
        inner join `tabItem Attribute Value` iav on iva.attribute_value = iav.attribute_value
        where iva.attribute = 'Size' {3} order by b.item_name,b.warehouse,b.size_group,iav.abbr) f) z where z.item_code REGEXP '.C' or z.item_code REGEXP '.A'""".format(conditions3,conditions,conditions4,conditions), as_dict=1)
    else:
